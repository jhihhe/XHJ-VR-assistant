...
### v2.7.4 (2026-03-16)
- **Fix**: Resolved an issue where the panorama upload counter showed 0 in some cases due to DOM structure variations. Enhanced recognition of status text within `div` and `span` containers.

### v2.7.3 (2026-03-16)
- **UI Adjustment**: Optimized the position of the panorama upload counter, moving it to the right side of the "Type" selection bar (next to radio buttons) for a more compact layout.
...
### v2.7.0 (2026-03-16)
- **UI Upgrade**: Revamped the survey upload counter with a **3D digital tube style**, simulating a real electronic display effect (inner shadow + glow).
- **Smart Interaction**: Added color gradient logic; the number color automatically transitions from **Red** to **Green** as the upload count increases (0 → 35), providing intuitive progress feedback.
