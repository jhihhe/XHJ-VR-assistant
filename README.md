...
![版本](https://img.shields.io/badge/版本-v2.7.4-brightgreen)
...
### v2.7.4 (2026-03-16)
- **修复**: 修复全景图上传计数器在某些场景下因 DOM 结构差异导致统计结果为 0 的问题。增强了对 `div` 和 `span` 容器内状态文本的识别。
...
- **优化**: 重构计数器渲染逻辑，复用 3D 样式组件。

### v2.7.1 (2026-03-16)
- **计数器优化**: 调整 3D 数码管计数器的变色逻辑，从 **9张** 起即显示浅绿色（Yellow-Green），并支持显示完整文本“已上传: XX 张”。
...
- **性能优化**: 合并多个 DOM 监听器，减少内存开销；优化图片计数逻辑。
- **稳定性**: 修复重复绑定点击事件的问题，提升长时间运行的稳定性。
