    ...
    m.controller('ctrl',['$scope',function($scope) {
        layui.use(['form', 'layer','upload','element'], function () {
            ...
            form.on('submit(submit)', function (data) {
                parent.location.href = "/houseadmin/pano/image.html?order_id=596736";
            });
            //普通图片上传
            ...
            //多图片上传
              ...
              //上传图片
                $(document).on("click", ".update", function(event){
                    ...
                    $.post("",{'order_id':596736,'image_name':image_name,'image_url':image_url,'image_nums':image_nums,'image_area':image_area,'storey':storey}, function (res) {
                        if (res.code > 0) {
                ...
                function up_img(name){
                    ...
                    doc.removeClass('update')
                    doc.addClass('layui-btn-danger')
                    $.post("",{'order_id':596736,'image_name':name,'image_url':image_url,'image_nums':image_nums,'image_area':image_area,'storey':storey}, function (res) {
                        if (res.code > 0) {
                            doc.removeClass('layui-btn-danger')
                            doc.text('上传成功')
                        } else {
                ...
                function up_imgs(id){
                    ...
                    doc.removeClass('update')
                    doc.addClass('layui-btn-danger')
                    $.post("",{'order_id':596736,'image_name':image_name,'image_url':image_url,'image_nums':image_nums,'image_area':image_area}, function (res) {
                        if (res.code > 0) {
                            doc.removeClass('layui-btn-danger')
                            doc.text('上传成功')
                        } else {
